...
@SpringBootTest(classes = GenericUpdateCommandListenerTestImpl.class)
class GenericUpdateCommandListenerTest {
  ...
  @MockBean
  UpdateCommandHandlerTestImpl mockCommandHandler;
  @MockBean
  InputValidationService inputValidationService;
  @MockBean
  ResponseMessageCreator responseMessageCreator;
  @Autowired
  private GenericUpdateCommandListenerTestImpl commandListener;
  ...
  void happyUpdatePath() {
    commandListener.update(KEY, mockRequest());
  ...
  @Test
  @DisplayName("Check if response is valid when DB error for update")
  void procedureErrorOnUpdate() {
    doThrow(new ProcedureErrorException("")).when(mockCommandHandler).update(any());
  ...
  @Test
  @DisplayName("Check if response is valid when NPE for update")
  void nullPointerExceptionOnUpdate() {
    doThrow(new NullPointerException("")).when(mockCommandHandler).update(any());
  ...
  void shouldReturnErrorStatusFromValidation() {
    ...
    var actualMessage = commandListener.update(KEY, mockRequest());
